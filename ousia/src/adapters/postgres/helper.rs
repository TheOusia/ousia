        ...
        match filter.mode.as_search().expect("search filter").comparison {
            ...
            NotIn => match &filter.value {
                IndexValue::Array(arr) => NativeTimestampBind::TextArray(
                    arr.iter()
                        .map(|e| match e {
                            IndexValueInner::String(s) => s.clone(),
                            IndexValueInner::Int(i) => i.to_string(),
                            IndexValueInner::Float(f) => f.to_string(),
                        })
                        .collect(),
                ),
                other => NativeTimestampBind::TextArray(vec![Self::index_value_text(other)]),
            },
